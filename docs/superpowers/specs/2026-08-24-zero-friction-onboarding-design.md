...
让不了解 Skill、Python、JSON 或 CLI 的京东同事完成一次安装后，只需向 Codex 描述生图需求。Codex 自动识别业务场景、补齐必要信息、预估调用量并选择现有能力执行。
...
Codex 在后台创建最小任务文件、运行 `estimate`，并用中文告诉用户预计文案与图片请求数。只有用户确认后才执行真实模型请求。现有高级 CLI 保留，但从首页主流程移到高级用法。
...
- 不自动执行产生 Oxygen 消耗的请求；请求量确认仍是强制门槛。
