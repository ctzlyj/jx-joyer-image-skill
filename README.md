...
> 请从同一仓库更新 JX Joyer Image Skill；先核对本地改动，保留我的输出、历史和已有运行环境，合并更新后运行离线检查，不要执行真实生图。
...
需求明确后，Codex 会直接开始执行，不再要求你确认调用次数。需要 Key 时，终端会提示：
...
- 自由工作台：文生图、多参考图编辑、按行或分隔符提示词队列、固定比例、自适应比例、逐图进度和失败项重试。
